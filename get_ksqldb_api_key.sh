...
cat etc/${KSQLDB_ID}_api_key.json | jq --raw-output  '"KSQLDB_API_KEY=\(.api_key)"' >> etc/vars.sh
cat etc/${KSQLDB_ID}_api_key.json | jq --raw-output  '"KSQLDB_API_SECRET=\(.api_secret)"' >> etc/vars.sh
